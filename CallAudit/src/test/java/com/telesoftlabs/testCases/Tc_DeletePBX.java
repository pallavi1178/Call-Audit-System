...
public class Tc_DeletePBX extends BaseClass{
	public static final Logger logger = Logger.getLogger(Tc_DeletePBX.class.getName());
	static
	{
	...
	public void deletePBX()
	{
		...
		try {
			logger.info("****** Started Deleting PBX ******");
			try {
			...
			delete.getDeletePBX(pbxName).click();
			...
			logger.info("PBX deleted Successfully");
		} catch (Exception e) {
			logger.severe("Failed to Delete PBX.........:( "+e.getMessage());
		}finally {
	...
	private void assertSnackbar()
	{
		AddPBX add = new AddPBX(driver);
		try {
			String expectedSnackbar = deletePbxSnackbar;
			String actualSnackbar = add.getSnackBar().getText();
			logger.info("Actual snackbar obtaining for deleting PBX is: "+actualSnackbar);
