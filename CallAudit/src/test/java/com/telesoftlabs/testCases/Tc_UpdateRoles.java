...
import com.telesoftlabs.pageObjects.UpdateRoles;

public class Tc_UpdateRoles extends BaseClass{
	public static final Logger logger = Logger.getLogger(Tc_UpdateRoles.class.getName());
	static
	{
	...
	{
		update = new UpdateRoles(driver);
		wait = new WebDriverWait(driver, Duration.ofSeconds(5));
		add = new AddRoles(driver);
		try {
			logger.info("****** Started Updating the Roles ******");
			add.getRolesPage().click();
			update.getUpdateRoleButton(roleName.toUpperCase()).click();
			logger.info("Updating Role: "+ roleName.toUpperCase());
			...
			wait.until(ExpectedConditions.visibilityOf(update.getSnackBar()));
			assertSnackbar();
			assertURL();			
			...
			logger.severe("Failed to Update Roles.....:( "+ e.getMessage());
		}
	}
	...
	private void assertSnackbar()
	{
		update = new UpdateRoles(driver);
		try {
			String expectedSnackbar = updateRolesSnackbar;
