...
public class Tc_DeleteBillingClass extends BaseClass{
	public static final Logger logger = Logger.getLogger(Tc_DeleteBillingClass.class.getName());
	static
	{
	...
	public void deleteBillingClass()
	{
		try {
			...
			logger.info("***** started Deleting Billing Class *****");
			try {
		    ...
		    delete.getDeleteBillingClassButton(billingClass).click();
            ...
            logger.info("Billing Class Deleted Successfully......");
		} catch (Exception e) {
	...
	private void assertSnackbar()
	{
		add = new AddRegion_BillingClass_BillingException(driver);
		try {
			String expectedSnackbar = deleteBillingClassSnackbar;
			String actualSnackbar = add.getSnackBar().getText();
			logger.info("Actual snackbar obtaining for deleting Billing Class is: "+actualSnackbar);
