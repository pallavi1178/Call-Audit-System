...
public class Tc_UpdateBillingClass extends BaseClass {
	public static final Logger logger = Logger.getLogger(Tc_UpdateBillingClass.class.getName());
	static
	{
	...
	public void updateBillingClass()
	{
		try {
			...
			logger.info("***** started Updating Billing Class *****");
			try {
		    ...
		    update.getUpdateBillingClassButton(billingClass).click();
            ...
            logger.info("Billing Class Updated Successfully......");
		} catch (Exception e) {
	...
	private void assertSnackbar()
	{
		add = new AddRegion_BillingClass_BillingException(driver);
		try {
			String expectedSnackbar = updateBillingClassSnackbar;
			String actualSnackbar = add.getSnackBar().getText();
			logger.info("Actual snackbar obtaining for Updating Billing Class is: "+actualSnackbar);
