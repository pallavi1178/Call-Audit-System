...
public class Tc_UpdateRegion extends BaseClass
{
	public static final Logger logger = Logger.getLogger(Tc_UpdateRegion.class.getName());
	static
	{
	...
	public void updateRegion()
	{
		try {
			...
			logger.info("***** started Updating Region *****");
			try {
		    ...
		    update.getUpdateRegionButton(countryName).click();
			...
			logger.severe("Failed to update Region......"+e.getMessage());
		}
		finally {
	...
	private void assertSnackbar()
	{
		add = new AddRegion_BillingClass_BillingException(driver);
		try {
			String expectedSnackbar = updateRegionSnackbar;
			String actualSnackbar = add.getSnackBar().getText();
			logger.info("Actual snackbar obtaining for updating region is: "+actualSnackbar);
